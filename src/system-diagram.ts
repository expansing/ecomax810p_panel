...
export function renderSystemDiagram(values: DiagramValues, entities: EntityMap, compact = false): string {
  if (compact) return renderCompactSystemDiagram(values, entities);
...
<svg class="systemDiagram" viewBox="0 0 960 350" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ecoMAX heating system status">
  ...
  <g class="outsideReading" transform="translate(480 27)">
    ...
    ${entityLink(entities.outside_temperature, `<text text-anchor="middle" dominant-baseline="central">OUTSIDE ${esc(values.outside)}</text>`)}
  </g>
  ...
  <g class="systemNode systemNode--boiler" transform="translate(52 100)">
    ...
    ${entityLink(entities.boiler_temperature, `<text class="nodeValue" x="20" y="82">${esc(values.boilerNow)}</text>`)}
    ${entityLink(entities.boiler_target_temperature, `<text class="nodeTarget" x="20" y="106">SETPOINT ${esc(values.boilerTarget)}</text>`)}
    <line class="nodeRule" x1="20" y1="126" x2="214" y2="126"/>
    <text class="nodeLabel" x="20" y="154">OUTPUT</text>
    ${entityLink(entities.boiler_load, `<text class="nodeDetail" x="20" y="179">${esc(values.boilerLoad)}</text>`)}
    <text class="nodeLabel" x="120" y="154">FUEL</text>
    ${entityLink(entities.fuel_level, `<text class="nodeDetail" x="120" y="179">${esc(values.fuelLevel)}</text>`)}
  ...
  <g class="systemNode systemNode--circuit" transform="translate(412 100)">
    ...
    ${entityLink(entities.mixer_temperature, `<text class="nodeValue nodeValue--medium" x="20" y="70">${esc(values.mixerNow)}</text>`)}
    ${entityLink(entities.mixer_target_temperature, `<text class="nodeTarget" x="106" y="70">TARGET ${esc(values.mixerTarget)}</text>`)}
  ...
  <g class="systemNode systemNode--dhw" transform="translate(412 225)">
    ...
    ${entityLink(entities.dhw_temperature, `<text class="nodeValue nodeValue--medium" x="20" y="70">${esc(values.dhwNow)}</text>`)}
    ${entityLink(entities.dhw_target_temperature, `<text class="nodeTarget" x="106" y="70">TARGET ${esc(values.dhwTarget)}</text>`)}
...
function renderCompactSystemDiagram(values: DiagramValues, entities: EntityMap): string {
  return `
<svg class="systemDiagram systemDiagram--compact" viewBox="0 0 360 322" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ecoMAX compact heating system status">
  ...
  <g class="outsideReading" transform="translate(120 14)">
    ...
    ${entityLink(entities.outside_temperature, `<text x="60" y="16" text-anchor="middle">OUTSIDE ${esc(values.outside)}</text>`)}
  </g>
  <g class="compactNode compactNode--boiler" transform="translate(14 52)">
    ...
    ${entityLink(entities.boiler_temperature, `<text class="compactValue" x="16" y="65">${esc(values.boilerNow)}</text>`)}
    ${entityLink(entities.boiler_target_temperature, `<text class="nodeTarget" x="116" y="65">SETPOINT ${esc(values.boilerTarget)}</text>`)}
  ...
  <g class="compactNode compactNode--circuit" transform="translate(14 184)">
    ...
    ${entityLink(entities.mixer_temperature, `<text class="compactValue" x="15" y="64">${esc(values.mixerNow)}</text>`)}
    ${entityLink(entities.mixer_target_temperature, `<text class="nodeTarget" x="15" y="84">TARGET ${esc(values.mixerTarget)}</text>`)}
  ...
  <g class="compactNode compactNode--dhw" transform="translate(186 184)">
    ...
    ${entityLink(entities.dhw_temperature, `<text class="compactValue" x="15" y="64">${esc(values.dhwNow)}</text>`)}
    ${entityLink(entities.dhw_target_temperature, `<text class="nodeTarget" x="15" y="84">TARGET ${esc(values.dhwTarget)}</text>`)}
